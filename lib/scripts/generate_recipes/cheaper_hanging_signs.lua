...
-- Generates cheaper hanging sign recipe for each wood type.
...
for rowIndex, row in ipairs(woodTypes) do
    ...
    local fileName = string.format("data/minecraft/recipe/%s_hanging_sign.json", woodType)
        ...
        file:write(string.format([[
{
        ...
        "result": {
            "id": "minecraft:%s_hanging_sign",
            "count": 12
        }
}
]], woodType, blockType, woodType))
        file:close()
        print(string.format("Generated: %s_hanging_sign.json", woodType))
    else
